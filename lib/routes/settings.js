 ...
 *   POST /settings/token  → body { token }：非空=加密保存（并清掉旧版明文配置），空串=清除
 */
 ...
 * @param {{ appCtx: object, dataDir: string, config?: object }} deps
 */
export function registerSettingsRoutes(app, { appCtx, dataDir, config } = {}) {
  ...
  app.post("/settings/token", async (c) => {
    ...
    try {
      if (!token) {
        await clearSecret({ dataDir, config });
        return c.json({ ok: true, tokenConfigured: false });
      }
      const r = await saveSecret({ dataDir, token, config });
      return c.json({
        ok: true,
        tokenConfigured: true,
        protection: r.protection,
        clearedLegacy: r.clearedLegacy,
      });
    } catch (e) {
      return c.json({ error: String((e && e.message) || e) }, 500);
    }
