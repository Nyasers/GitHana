 ...
 *
 * 设计：手动按钮不另写一套逻辑，直接调用 Agent 调的那几个**工具模块本体**
 * （tools/gpg-keygen.js 等）——同一个 execute，两个入口。这样 Agent 路径和
 * 用户自助路径的行为、校验、报错文案永远只有一份。
 *
 * 端点：
...
export function registerActionRoutes(app, { appCtx, dataDir } = {}) {
  // 生成 / 轮换隔离 GPG 签名密钥：复用 Agent 的 gpg_keygen 工具本体。
   ...
   * 与 gpg_keygen 内部推导用的是同一个纯函数（tools/lib/identity.js），两边不会跑偏。
