 ...
 * ## 为什么必须加密
 *
 * 明文落盘的问题是真的：旧版把 token 放在 preferences.json（settings_contributions），
 * 而 .hanako 下的 ACL 对 Authenticated Users 是 Modify。所以要加密，且不回落到明文。
 *
 * ## 后端可插拔（当前只有 Windows）
 *
...
import { getToolDataDir, getToolConfigValue, setToolSecretToken } from "../tools/lib/context.js";
...
export async function loadSecretIntoContext(dataDir) {
  ...
  const legacy = getToolConfigValue("token");
  if (legacy) {
    setToolSecretToken(legacy);
    return { source: "legacy-config", configured: true };
  }
  setToolSecretToken("");
...
/**
 * 保存令牌：走后端加密落盘，随后清掉旧版明文配置（迁移）。
 * 顺序有意如此：先确认保护区写入成功，再抹本地明文，避免出现「两边都没有」的窗口。
 * 没有可用后端时**直接失败**，不写明文。
 */
export async function saveSecret({ dataDir, token, config } = {}) {
  const dir = dataDir || getToolDataDir();
  const backend = pickBackend();
  ...
  try {
    ...
    /* 清不掉只是残留一份旧明文，不影响本次保存 */
  }
  setToolSecretToken(token);
  return { clearedLegacy, protection: backend.alg, backend: backend.id };
}

/** 清除令牌：删记录 + 清旧版明文配置 + 清进程缓存（keyring 形态时 backend.clear 由各后端实现）。 */
export async function clearSecret({ dataDir, config } = {}) {
  const backend = pickBackend();
  ...
  try {
    if (config && typeof config.set === "function") await config.set("token", "");
  } catch {
    /* 忽略 */
  }
  setToolSecretToken("");
...
export function secretInfo(dataDir) {
  ...
  if (getToolConfigValue("token")) {
    return {
      configured: true,
      protection: "plaintext(legacy)",
